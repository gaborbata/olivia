var Module = function (n) {
    ...
    function i(n) {
      return (
        f || "function" != typeof fetch
          ...
          : fetch(nr, { credentials: "same-origin" })
            .then(function (n) {
              if (!n.ok)
                throw "failed to load wasm binary file at '" + nr + "'";
  ...
  var nR = (n.___wasm_call_ctors = function () {
    ...
    nI = (n.___embind_register_native_and_builtin_types = function () {
      return (nI = n.___embind_register_native_and_builtin_types =
  ...
  if (message.data.imageBuffer) {
    mod
      .then(function (module) {
        ...
        postMessage({ imageData: decodedImage, meta: message.data.meta });
      })
      .catch(function (error) {
        postMessage({ error: error.message, meta: message.data.meta });
      });
  } else {
    postMessage({ error: "Missing image data", meta: message.data.meta });
  }
});
