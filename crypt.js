...
var ARC = require('./asset/js/arc.js');
...
var App = App || (function () {
  ...
  function encrypt(message, password, name, date) {
    ...
    console.log('ARC:', name.padEnd(14, ' '), date.toISOString(), ARC.headerSize + encryptedFileBytes.length);
  ...
  function decrypt(encrypted, password) {
    ...
    console.log('ARC:', arc.header.filename.padEnd(14, ' '), arc.header.timestamp, (arc.checksum && arc.sizeMatches) ? ' OK' : ' ERROR');
  ...
  fs.readdir(source, function (err, files) {
    files.forEach(function (file, index) {
      ...
      if (name == imagesName || /*name == appConfigName ||*/ (!fs.existsSync(target + prefix + name + extension) && fs.lstatSync(source + file).isFile()) && name.length > 0) {
        ...
        var enc = App.encrypt(CryptoJS.enc.Latin1.parse(originalString), password, name.replaceAll('_', ''), date);
